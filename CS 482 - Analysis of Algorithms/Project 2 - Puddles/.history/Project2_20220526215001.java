...
public class Project2 {
    ...
    // --------------------------------
    public static int countPuddles(int[][] grid) {
    ...
    public static void Islands(int[][] grid, int i, int j, boolean[][] visited) {
        if (i < 0 || i >= grid.length || j < 0 || j >= grid[0].length || grid[i][j] == 1 || visited[i][j] == true) {
            return;
        }
        visited[i][j] = true;
        // Travel in all direction: north, east, south and west.
        Islands(grid, i - 1, j, visited);// North
        Islands(grid, i, j + 1, visited);// East
        Islands(grid, i + 1, j, visited);// South
        Islands(grid, i, j - 1, visited);// West
    }
    // --------------------------------
