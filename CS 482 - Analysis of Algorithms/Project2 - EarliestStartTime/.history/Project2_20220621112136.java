...
return index;
    ...
    public static int findCardinality() {
        ...
        for (int i = 0; i < N; i++) {
            if (i != earliest) {
                if (intervals[i][0] < intervals[earliest][1] || (intervals[i][1] > intervals[earliest][0] && intervals[i][0] < intervals[earliest][0])) {
                    System.out.println((intervals[i][0] < intervals[earliest][1]) + " " + intervals[i][0] + " " +  intervals[earliest][1]);
                    result--;
                }
            }
        }
...
//add reuslting non-overlapping intervals to a diff array, and nuke the ones that overlap with another that is the current earliest