...
public static boolean checkStable(int currManID, int currWomanIndex) {
...
//return false if wm == matchings[cmi][1]
 ...
 for (int i =0; i<N;i++) {
     if (currWomanIndex == mensDesire[currManIndex][i]) {
         wm = i;
     }
 }

for (int i =0; i<N;i++) {
    if (currManIndex == womensDesire[currWomanIndex][i]) {
        mm = i;
    }
}

if (wm == matchings[currManIndex][1]) {
    return true;
}
