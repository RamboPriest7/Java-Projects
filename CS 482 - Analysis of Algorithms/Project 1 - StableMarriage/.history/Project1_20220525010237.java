...
//return false if wm == matchings[cmi][1]
 ...
 for (int i =0; i<N;i++) {
     if (currWomanID == mensDesire[currManID][i]) {
         wm = i;
     }
 }

for (int i =0; i<N;i++) {
    if (currManID == womensDesire[currWomanID][i]) {
        mm = i;
    }
}

if (wm == matchings[currManID][1]) {
    return true;
}


//now find the preference index, desIndex of cmi in womensDesires[currWomanID]
//and also find the preference index of cwi in mensDesires[currWomanID]  
