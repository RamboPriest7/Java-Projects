...
public class Project1 {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (int i = 1; i <= N; i++) {
            ...
            for (int q = 0; q < N; q++) {
                if (matchings[q][i-1] == mensDesire[i - 1][q]) {
                    currMenMatch = q;
                    System.out.println(q);
                }
            }
            for (int j = 1; j <= N; j++) {
                if (j != matchings[i - 1][1]) {
                    ...
                    // System.out.println( "In Array: " + mensDesire[i-1][0] + " " +
                    // womensDesire[j-1][0]);
                    for (int k = 0; k < currMenMatch-1; k++) {
                        if (mensDesire[i-1][k] == j && womensDesire[j-1][k] == i) {
                            notStable++;
                            }
                    }
