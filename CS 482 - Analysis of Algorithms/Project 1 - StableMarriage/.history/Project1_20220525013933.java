...
public class Project1 {
 ...
 System.out.println("currWomanID, currManID == " + currWomanID + " " + currManID );
        if (currWomanID == matchings[currManID - 1]) {
            return true;
        }

        int pref_of_curr_male_check = -1, pref_of_curr_female_check = -1, pref_of_curr_male_match = -1, pref_of_curr_female_match = -1;

        for (int i = 0; i < N; i++) {
            if (matchings[currManID - 1] == mensDesire[currManID - 1][i]) {
                pref_of_curr_male_check = i;
            }
        }

        for (int i = 0; i < N; i++) {
            if (currManID == womensDesire[currWomanID - 1][i]) {
                pref_of_curr_female_check = i;
            }
        }
        ...
        for (int i = 0; i < N; i++) {
            if (currManID == womensDesire[currWomanID - 1][i]) {
                pref_of_curr_male_match = i;
            }
        }

        for (int i = 0; i < N; i++) {
            if (currWomanID == mensDesire[currManID - 1][i]) {
                pref_of_curr_female_match = i;
            }
        }

        System.out.println("wm, mm == " + wm + " " + mm );
        return ((pi_cmi <= wm) && (pi_cwi <= mm));
