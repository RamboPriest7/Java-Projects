class BlackJack {
    ...
    public void dealCards() { // to add cards in to the hand. this should also be used for Hit in game
        ...
        int newCardNum; // later when program is working, use this random, set that to a int and get
                        // that int from Deck
        for (int i = 1; i <= 2; i++) {
            ...
            System.out.println(
                    newCard.getSuit() + " " + newCard.getNumValue() + "card name is this: " + newCard.getCardName());
            ...
            System.out.println(newCard2.getSuit() + " " + newCard2.getNumValue() + " card name is this: "
                    + newCard2.getCardName());
            // theDeck.deleteCard(newCardNum);
            newCard2.printCard();
            ...
            if ((playerScore == newCard2.getNumValue() + 1 ^ playerScore - newCard2.getNumValue() == 1)
                    && playerScore < 11) {
                playerScore += 10;
                System.out.println("10 has been added to player score");
            }
            if ((dealerScore == newCard.getNumValue() + 1 ^ dealerScore - newCard.getNumValue() == 1)
                    && dealerScore < 11) {
                dealerScore += 10;
    ...
    public boolean checkLose() {
        if ((playerScore != 21 && dealerScore == 21) || (dealerScore > playerScore) && (dealerScore < 22)
